...
This repository implements a Fourier-domain PTA detection statistic together with right-tail p-value evaluation under H_0 using Imhof’s method (generalized \chi^2). It also includes a lightweight get_phi implementation (PTA_Lite) to avoid enterprise-level constraints, while keeping the same \phi-structure: per-pulsar RN + a common GW process with optional Hellings–Downs correlations.
...
PTA_Lite constructs Fourier prior variances in a real (cos/sin) basis:
	...
	•	Modes:
	•	mode="curn" (H0): returns a list (len=P) of per-pulsar diagonals
...
where \Gamma is the Hellings–Downs ORF from sky positions (phi, theta).
